          ...
          (concat
            (if-not (schema/attr-is-card-one? attr-conf)
              (for [old old-set
                ...
                [:db/retract eid-3df k (schema/maybe-encode-id flat-schema k old)]))
            (for [new new-set
                  :when (not (nil? new))
                  :when (not (contains? old-set new))]
              [:db/add eid-3df k (schema/maybe-encode-id flat-schema k new)])))))))

(defn- process-put-doc
  "submits docs in put tx"
  [flat-schema crux-db snapshot acc new-doc
  ...
  (let [_ (log/debug "NEW-DOC:" (pr-str new-doc))
        ...
        doc-changed-triplets (calc-changed-triplets eid-3df flat-schema old-doc new-doc)]
    (into acc doc-changed-triplets)))

(defn- sanitise-tx-ops [flat-schema tx-ops]
  (reduce
    (fn [acc [tx-type doc-or-id :as tx-op]]
      (case tx-type
        :crux.tx/put
        (if (and (map? doc-or-id) (schema/matches-schema? flat-schema doc-or-id))
          (conj acc tx-op)
          acc)
        acc))
    []
...
(defn submit-tx-log-entry
  [crux-node conn df-db flat-schema
   {:keys [crux.api/tx-ops crux.tx/tx-time crux.tx/tx-id] :as tx}]
  (log/debug "CRUX_TX:" (pr-str tx))
  (let [crux-db (api/db crux-node tx-time tx-time)
        clean-tx-ops (sanitise-tx-ops flat-schema tx-ops)]
    (with-open [snapshot (api/new-snapshot crux-db)]
      (let [new-df-txes
            (reduce
              (fn [acc [tx-type doc-or-id]]
                (case tx-type
                  :crux.tx/put (process-put-doc flat-schema crux-db snapshot acc doc-or-id tx)
                  acc))
              []
              clean-tx-ops)]
...
(defn submit-crux-query-results
  [{:keys [conn df-db flat-schema] :as df-listener}
   crux-query-results]
  (let [df-compatible-maps (mapv (partial schema/prepare-map-for-3df flat-schema) crux-query-results)]
